...
> 若无特别说明，所有源码均采用 **.NET 8 + MonoGame 3.8** 环境。
...
* 目标：在 MonoGame 基础上做一款高性能、可扩展、Ren’Py 级易用的视觉小说引擎。
...
├─ SakiEngine.Content/       ← 剧本脚本 + 资源（编译成 DLL）
├─ SakiEngine.Launch/          ← 可执行项目 / 启动器
├─ MiniGames/                ← 可选独立小游戏 DLL 插件
...
## 13. MiniGame 插件
* DLL 实现 `IGameModule`（`Initialize/Update/Draw/Dispose`）。  
* `MiniGameHostMenu` 加载并包在菜单栈中；输入独立。  
* Dev 可用 Roslyn `.csx` 热编译；正式版走预编译 DLL。
...
3. 译者填 `en / ja` 列。  
4. `loc-compile` → `Lang_xx.cs`（Release）或 CSV 热加载（Debug）。  
