...
void main() {
  ...
  test(
    'animation final properties preserve preset and last keyframe state',
    () {
...
''');
      ...
      final result = AnimationManager.resolveFinalProperties('bigjump', {
        'xcenter': 0.5,
        'ycenter': 0.7,
        'scale': 1.3,
        'alpha': 1.0,
...
''');
      ...
      await manager.startTestScript(
        ScriptNode([
          SayNode(character: 'x', dialogue: '放大并停住。', animation: 'bigjump'),
          SayNode(character: 'x', dialogue: '现在缩回去。', animation: 'shrink'),
        ]),
  ...
  testWidgets(
    'dialogue history captures a running finite animation at its last frame',
    (tester) async {
      AnimationManager.loadAnimationsFromStringForTesting('''
zoombig
ease 0.2 scale+0.5
''');
      ...
      await manager.startTestScript(
        ScriptNode([
          SayNode(character: 'x', dialogue: '突然凑近。', animation: 'zoombig'),
          SayNode(character: 'x', dialogue: '下一句。'),
        ]),
        initialState: GameState(
          characters: {
            'xiayo2': CharacterState(
      ...
      final animatedEntry = manager.getDialogueHistory().single;
      final snapshottedProperties = animatedEntry
          .stateSnapshot
          .currentState
          .characters['xiayo2']!
          .animationProperties!;
      expect(snapshottedProperties['scale'], closeTo(1.8, 0.0001));
      ...
      final retainedProperties = manager
          .getDialogueHistory()
          .first
          .stateSnapshot
          .currentState
          .characters['xiayo2']!
          .animationProperties!;
      expect(retainedProperties['scale'], closeTo(1.8, 0.0001));
    },
  );

  testWidgets('narration tail animation is captured at its last frame', (
    tester,
  ) async {
    AnimationManager.loadAnimationsFromStringForTesting('''
zoombig
...
''');
    ...
    await manager.startTestScript(
      ScriptNode([
        SayNode(
          ...
          tailCharacter: 'x',
          tailPose: 'pose1',
          tailExpression: 'happy',
          tailAnimation: 'zoombig',
        ),
        SayNode(character: 'x', dialogue: '真的想知道？'),
      ]),
      initialState: GameState(
        characters: {
          'xiayo2': CharacterState(
    ...
    final properties = manager
        ...
        .characters['xiayo2']!
        .animationProperties!;
    expect(properties['scale'], closeTo(1.8, 0.0001));
  ...
  test(
    'explicit dialogue position without animation resets retained transform',
    () async {
      final manager = GameManager();
      addTearDown(manager.dispose);
      manager.characterConfigs['x'] = CharacterConfig(
      ...
      await manager.startTestScript(
        ScriptNode([
          SayNode(character: 'x', dialogue: '回到中央舞台位。', position: 'center'),
        ]),
        initialState: GameState(
          characters: {
            'xiayo2': CharacterState(
