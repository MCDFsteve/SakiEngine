...
part 'game_manager_choice_seek.dart';
...
class GameManager {
  ...
  bool _isSeekingNextChoice = false;
  int _choiceSeekNodesRemaining = 0;
  int _choiceSeekRevision = 0;
  final _choiceSeekLoopingAnimations = <String, (String, String)>{};
  bool get _skipPresentation => _isFastForwardMode || _isSeekingNextChoice;
  String? _activeCgTransitionType;
  Completer<void>? _cgTransitionCompleter;
  static const Duration _defaultCgTransitionDuration = Duration(
    milliseconds: 300,
  );
  ...
  int _characterPresentationRevision = 0;
  bool _pendingCharacterReflow = false;

  void _cancelCharacterPresentation() {
    _characterPresentationRevision++;
    _isProcessing = false;
    _pendingCharacterReflow = false;
    _characterPositionAnimator?.dispose();
    _characterPositionAnimator = null;
  ...
  /// 检查是否需要创建自动存档
  Future<void> _checkAndCreateAutoSave(
    int scriptIndex, {
    String? reason,
  }) async {
    if (_disableRuntimeSideEffectsForTesting || _isSeekingNextChoice) {
      return;
    }
    try {
      ...
      if (nodeId != null && nodeType != null) {
        ...
        final actualAutoSaveId = await _flowchartManager.createAutoSaveForNode(
          nodeId,
          saveSlot,
        );

        // 解锁节点，使用实际的 autoSaveId
        await _flowchartManager.unlockNode(
          nodeId,
  ...
  Future<void> _checkChapterEndAutoSave(int sceneIndex) async {
    if (_isSeekingNextChoice || _disableRuntimeSideEffectsForTesting) return;
      ...
      if (node is JumpNode) {
        ...
        if (targetChapterMatch != null) {
          ...
          if (targetChapter != currentChapter) {
            final lastSceneIndex = _findLastSceneWithDialogueBeforeChapterEnd(
              i,
            );
            if (kEngineDebugMode) {
  ...
  String? _findNearestLabel(int index) {
    var low = 0;
    var high = _sortedLabelEntries.length - 1;
    String? result;
    while (low <= high) {
      final middle = low + ((high - low) >> 1);
      final entry = _sortedLabelEntries[middle];
  ...
  Future<void> _checkAndAnimatePoseAttributeChanges({
    ...
    // 获取旧的和新的pose配置
    final oldPoseConfig = oldPositionId != null
        ? _poseConfigs[oldPositionId]
        : null;
    final newPoseConfig = newPositionId != null
        ? _poseConfigs[newPositionId]
        : null;
    ...
    await _characterPositionAnimator!.animateAttributeChanges(
      ...
      onUpdate: (attributesMap) {
        if (_disposed || revision != _characterPresentationRevision) return;
        // 更新角色的动画属性
        final updatedCharacters = Map<String, CharacterState>.from(
  ...
  Future<void> _checkAndAnimateCharacterPositions(
    ...
    bool reconcileCurrentPositions = false,
  }) async {
    final revision = _characterPresentationRevision;
    ...
    var positionChanges = CharacterAutoDistribution.calculatePositionChanges(
      _currentState.characters,
      newCharacters,
      _poseConfigs,
      _poseConfigs,
    ...
    if (reconcileCurrentPositions) {
      ...
      positionChanges = [
        for (final entry in newCharacters.entries)
          if (distributed['${entry.key}_auto_distributed'] case final target?)
            CharacterPositionChange(
              characterId: entry.key,
              fromX:
                  entry.value.animationProperties?['xcenter'] ?? target.xcenter,
              toX: target.xcenter,
            ),
      ];
    } else {
      // Layout changes start from the displayed position, including an
      // interrupted reflow, rather than jumping to the old nominal slot.
      positionChanges = [
    ...
    final layoutChanged = positionChanges.isNotEmpty;
    positionChanges = positionChanges
        .where((change) => (change.fromX - change.toX).abs() > 0.001)
        .toList();

    void applyPositions(Map<String, double> positions) {
      if (_disposed || revision != _characterPresentationRevision) return;
      final updatedCharacters = Map.of(_currentState.characters);
      for (final entry in positions.entries) {
        final character = updatedCharacters[entry.key];
        if (character != null) {
          _activeCharacterAnimations[entry.key]?.rebaseXCenter(entry.value);
          updatedCharacters[entry.key] = character.copyWith(
            animationProperties: {
    ...
    if (positionChanges.isNotEmpty) {
      ...
      if (_skipPresentation || _tickerProvider == null) {
        applyPositions({
          for (final change in positionChanges) change.characterId: change.toX,
        });
        return;
      }
  ...
  int estimateCurrentDialogueOccurrenceInFile({
    ...
    final safeTargetIndex = targetNodeIndex
        .clamp(0, _script.children.length - 1)
        .toInt();
  ...
  MapEntry<String, CharacterState>? _resolveTailTargetCharacterState(
    ...
    final targetKey = _resolveCharacterRenderKey(
      resolvedAlias,
  ...
  bool _applyNarrationTailCharacterDiff({
    ...
    final targetConfig = _resolveCharacterIdentityByAliasOrResourceId(
      targetAlias,
    )?.config;
    ...
    if (animation != null && animation.isNotEmpty) {
      if (_skipPresentation) {
        ...
        _playCharacterAnimation(targetKey, animation, repeatCount: repeatCount);
      }
    }
    if (kEngineDebugMode) {
      print(
  ...
  ({String apiName, Map<String, String> params})? _parseInlineApiToken(
    String rawToken,
  ) {
    ...
    if (leftParenIndex > 0 && payload.endsWith(')')) {
      ...
      return (apiName: apiName, params: params);
    }
    ...
    // 语法4: api<name>
    return (apiName: payload, params: params);
  }
  ...
  Future<void> _applyInlineApiTokenForSay({
    ...
    if (_isSeekingNextChoice && result.stateAfterWait != null) {
  ...
  Duration get cgRendererTransitionDuration => _activeCgTransitionType == 'diss'
      ? _dissCgTransitionDuration
      : _defaultCgTransitionDuration;

  /// Called by the CG renderer after its current image transition completes.
  void notifyCgRendererTransitionCompleted() {
    final completer = _cgTransitionCompleter;
    if (_activeCgTransitionType == 'diss' &&
        completer != null &&
  ...
  Future<void> startTestScript(
    ...
    Map<String, CharacterConfig>? characterConfigs,
    ...
    // Keep legacy fixtures that configure the manager before starting a script.
    _characterConfigs = characterConfigs ?? _characterConfigs;
    _poseConfigs = poseConfigs ?? _poseConfigs;
    _script = script;
  ...
  void _buildLabelIndexMap() {
    final nativeLabels = _scriptMerger.nativeLabelIndices;
    if (nativeLabels.isNotEmpty) {
      _labelIndexMap = Map<String, int>.from(nativeLabels);
    } else {
      _labelIndexMap = {};
      for (int i = 0; i < _script.children.length; i++) {
        final node = _script.children[i];
    ...
    _sortedLabelEntries = _labelIndexMap.entries.toList(growable: false)
  ...
  /// Advance through directing and story state without presenting intermediate frames.
  Future<bool> jumpToNextChoice() => _seekNextChoice();

  Future<void> jumpToLabel(String label) async {
    if (_disposed || _isSeekingNextChoice) return;
    if (!_disableRuntimeSideEffectsForTesting) {
      await MusicManager().stopVoice();
    }
    // 在合并的脚本中查找标签
  ...
  void executeScriptAfterMovie() {
    if (_disposed || _isSeekingNextChoice || _currentState.movieFile == null) {
      return;
    }
  ...
  Future<void> _executeScript() async {
    ...
    try {
      ...
      if (presentationRevision == _characterPresentationRevision) {
        _isProcessing = false;
        if (!_disposed && _pendingCharacterReflow) {
          _pendingCharacterReflow = false;
          unawaited(
            _checkAndAnimateCharacterPositions(
              _currentState.characters,
              reconcileCurrentPositions: true,
            ),
          );
        }
      }
  ...
  Future<void> _processScriptNodes() async {
    ...
    while (!_disposed &&
        presentationRevision == _characterPresentationRevision &&
        _scriptIndex < _script.children.length) {
      final node = _script.children[_scriptIndex];
      final currentNodeIndex = _scriptIndex; // 保存当前节点索引

      if (_isSeekingNextChoice) {
        if (--_choiceSeekNodesRemaining < 0) {
          throw StateError('Choice seek exceeded the script execution limit');
        }
      }
      ...
      if (node is BackgroundNode) {
        ...
        if (_isSeekingNextChoice ||
          ...
          if (_isSeekingNextChoice && !isSameBackground) {
          ...
          if (_isSeekingNextChoice) _settleSceneAnimationForChoiceSeek();
          ...
          // 等待前消费当前scene及已合并的fx，避免计时结束后重复执行。
          _scriptIndex += sceneFilter != null ? 2 : 1;

          // 快进模式下跳过计时器
          if (!_skipPresentation && node.timer != null && node.timer! > 0) {
      ...
      if (node is MovieNode) {
        ...
        if (!_isSeekingNextChoice &&
            _isFastForwardMode &&
            _containsChapter(node.movieFile)) {
        ...
        if (_skipPresentation ||
            ...
            isInitialMovie ||
            isSameMovie) {
          if (_isSeekingNextChoice) _choiceSeekLoopingAnimations.clear();
          // 直接切换到视频
          _currentState = _currentState.copyWith(
            movieFile: node.movieFile,
            clearMovieFile: _isSeekingNextChoice,
      ...
      if (node is AnimeNode) {
        ...
        // 快进模式下跳过anime显示
        if (!_skipPresentation || (_isSeekingNextChoice && node.keep)) {
          // 正常模式下显示anime
          _currentState = _currentState.copyWith(
      ...
      if (node is ShowNode) {
        ...
        if (characterConfig != null) {
          ...
          positionId = node.position ?? characterConfig.defaultPoseId ?? 'pose';
        ...
        final latestCharacter =
            newCharacters[finalCharacterKey] ?? currentCharacterState;
        newCharacters[finalCharacterKey] = latestCharacter.copyWith(
          resourceId: resourceId,
          pose: targetPose,
          expression: targetExpression,
          isFadingOut: false,
      ...
      if (node is CgNode) {
        ...
        // CgSlotWidget 使用 CharacterCompositeCache 取首帧。先在状态切换前
        // 等待同一份合成结果，保证 diss 的 800ms 从真实 CG 已可绘制时开始，
        // 而不是把异步合成时间算进转场并提前清掉旧场景。
        if (!_isSeekingNextChoice && !_disableRuntimeSideEffectsForTesting) {
          await CharacterCompositeCache.instance.preload(
            resourceId,
            newPose,
        ...
        if (_isSeekingNextChoice) {
          _choiceSeekLoopingAnimations.removeWhere(
            (key, value) => key != finalCharacterKey || value.$1 != resourceId,
          );
        }
        final requestedTransition = node.transitionType?.toLowerCase();
        final useDirectDissolve =
            !_skipPresentation &&
            _context != null &&
            (requestedTransition == 'diss' ||
                requestedTransition == 'dissolve');
        _activeCgTransitionType = useDirectDissolve ? 'diss' : null;
        _cgTransitionCompleter = useDirectDissolve ? Completer<void>() : null;

        // diss 必须保留上一场景作为交叉淡化底图。旧实现先清空背景，
        // CG 首帧又从透明开始，因此会在两者之间暴露 Scaffold 的黑色。
        // 其他路径继续保持 CG 的 scene 式独占语义。
        _currentState = _currentState.copyWith(
          everShownCharacters: _everShownCharacters,
          clearBackground: !useDirectDissolve,
          clearMovieFile: true,
          clearCharacters: !useDirectDissolve,
          clearSceneFilter: !useDirectDissolve,
          clearSceneLayers: !useDirectDissolve,
          clearSceneAnimation: !useDirectDissolve,
        );
        ...
        if (useDirectDissolve) {
          // 目标 CG 已在上方完成预合成，因此渲染器可以立即开始淡化。
          // 等 CG 完全不透明后再移除旧场景；画面不会经过黑色中间帧，
          // 同时脚本也像 scene `with diss` 一样等待转场完成。
          final transitionCompleter = _cgTransitionCompleter!;
          await transitionCompleter.future.timeout(
            _dissCgTransitionDuration + const Duration(milliseconds: 400),
            onTimeout: () {},
          );
          _currentState = _currentState.copyWith(
      ...
      if (node is HideNode) {
        ...
        if (character != null) {
          if (node.immediate || _isSeekingNextChoice) {
            _choiceSeekLoopingAnimations.remove(hideKey);
      ...
      if (node is ConditionalSayNode) {
        ...
        if (shouldAutoRenderDialogueCharacter && node.character != null) {
          ...
          if (currentCharacterState != null) {
            ...
            if (existingAnimController != null &&
              ...
              inheritedAnimationProperties = Map.from(
                currentCharacterState.animationProperties!,
              );
            ...
            _currentState = _currentState.copyWith(
              characters: {
                ..._currentState.characters,
                finalCharacterKey: newCharacters[finalCharacterKey]!,
              },
              everShownCharacters: _everShownCharacters,
            );
            _emitCurrentState();
        ...
        } else if (shouldAutoRenderDialogueCharacter &&
            tailSpeakerAlias != null &&
            tailSpeakerAlias.isNotEmpty) {
        ...
        if (_activeNvlContext != _NvlContextMode.none) {
          ...
          if (followingMenuNodeIndex != null) {
            ...
            // 分支选择前创建自动存档
            await _checkAndCreateAutoSave(
              followingMenuNodeIndex,
              reason: '分支选择',
            );
      ...
      if (node is SayNode) {
        ...
        final shouldDirectToFollowingMenu =
      ...
      if (node is JumpNode) {
        if (node.isConditional) {
          final currentValue = await GlobalVariableManager().getBoolVariable(
            node.conditionVariable!,
            defaultValue: false,
          );
          if (currentValue != node.conditionValue) {
        ...
        if (_isSeekingNextChoice) {
          final targetIndex = _labelIndexMap[node.targetLabel];
          if (targetIndex == null) return;
          _scriptIndex = targetIndex;
          continue;
        }
        _scriptIndex++;
        _isProcessing = false;
      ...
      if (node is PauseNode && _skipPresentation ||
          node is ShakeNode && _isSeekingNextChoice) {
        _scriptIndex++;
        continue;
      }
      ...
      if (node is ShakeNode) {
        ...
        Timer(Duration(milliseconds: (duration * 1000).round()), () {
          if (_disposed || navigationRevision != _choiceSeekRevision) {
            return;
          }
          _currentState = _currentState.copyWith(
            isShaking: false,
  ...
  Future<void> _transitionToNewMovie(
    String movieFile, [
    SceneFilter? sceneFilter,
    List<String>? layers,
      ...
      await SceneTransitionEffectManager.instance.transition(
        ...
        onMidTransition: () async {
          ...
          _currentState = _currentState.copyWith(
            ...
            sceneAnimationProperties: animation != null
                ? <String, double>{}
                : null,
  ...
  Future<void> _transitionToNewBackground(
    ...
    if (!ColorBackgroundRenderer.isValidHexColor(newBackground)) {
      try {
        ...
        if (assetPath == null) {
          assetPath = await AssetManager().findAsset(
            'backgrounds/${newBackground.replaceAll(' ', '-')}',
          );
        }
    ...
    if (effectType == TransitionType.diss) {
      ...
      if (_currentState.background != null) {
        ...
        final oldBgPath = await AssetManager().findAsset(
    ...
    if (effectType == TransitionType.fade) {
      ...
      await SceneTransitionEffectManager.instance.transition(
        ...
        onMidTransition: () {
          if (_disposed || navigationRevision != _choiceSeekRevision) return;
          //////print('[GameManager] scene转场中点 - 切换背景到: $newBackground');
          // dissolve 会先用完整旧场景帧盖住底层，再在这里提交新状态并
          // 捕获完整目标场景；其他效果仍在各自的视觉中点切换状态。
          if (effectType != TransitionType.diss) {
      ...
      // dissolve 的目标状态已经在冻结旧帧下提交，这里不需要重复更新。
    }
  ...
  Future<void> _checkAndRestoreSceneAnimation({
    ...
    if (lastBackgroundNode != null && lastBackgroundNode.animation != null) {
      ...
      _startSceneAnimation(
        lastBackgroundNode.animation!,
        lastBackgroundNode.repeatCount,
      );
  ...
  Map<String, double>? _buildCharacterAnimationBaseProperties(
    String characterId,
    CharacterState characterState, {
    required bool isCgCharacter,
  }) {
    if (isCgCharacter) {
      return {
        'xcenter': 0.0,
    ...
    if (poseConfig == null) return null;
  ...
  void _applyCharacterAnimationFinalState(
    ...
    var isCgCharacter = false;
    var characterState = _currentState.characters[characterId];
    if (characterState == null) {
      characterState = _currentState.cgCharacters[characterId];
      isCgCharacter = characterState != null;
    }
    if (characterState == null) return;
    if (_isSeekingNextChoice) {
      if (repeatCount == 0) {
        _choiceSeekLoopingAnimations[characterId] = (
          characterState.resourceId,
          animationName,
        );
      } else {
        _choiceSeekLoopingAnimations.remove(characterId);
      }
    }

    final baseProperties = _buildCharacterAnimationBaseProperties(
      characterId,
      characterState,
      isCgCharacter: isCgCharacter,
    );
    ...
    if (finalProperties == null) return;

    if (isCgCharacter) {
      final newCgCharacters = Map.of(_currentState.cgCharacters);
      newCgCharacters[characterId] = characterState.copyWith(
        animationProperties: finalProperties,
      );
      _currentState = _currentState.copyWith(
        cgCharacters: newCgCharacters,
  ...
  void removeCharacterAfterFadeOut(String characterId) {
    ...
    if (fadingCharacter == null || !fadingCharacter.isFadingOut) {
      // Ignore an old hide callback if dialogue/show has revived this slot.
      return;
    }

    final oldCharacters = _currentState.characters;
    final oldDistribution = CharacterAutoDistribution.calculateAutoDistribution(
      oldCharacters,
      _poseConfigs,
      oldCharacters.keys.toList(),
    );
    final newCharacters = Map.of(oldCharacters)..remove(characterId);
    for (final entry in newCharacters.entries) {
      final oldPose = oldDistribution['${entry.key}_auto_distributed'];
      if (oldPose != null) {
        // Keep the rendered position while the cast changes. Reflow must only
        // change x; zoom, jump height, opacity, and other script transforms stay.
        newCharacters[entry.key] = entry.value.copyWith(
          animationProperties: {
            ...?entry.value.animationProperties,
            'xcenter':
                entry.value.animationProperties?['xcenter'] ?? oldPose.xcenter,
          },
        );
      }
    }
    ...
    if (_isProcessing) {
      // A show/pose command may be awaiting this same animator. Remove the
      // faded sprite now, but reflow only after that command commits its cast.
      _pendingCharacterReflow = true;
    } else {
      unawaited(
        _checkAndAnimateCharacterPositions(
          newCharacters,
  ...
  Future<bool> getBoolVariable(String name, {bool defaultValue = false}) async {
    return await GlobalVariableManager().getBoolVariable(
      name,
  ...
  bool getBoolVariableSync(String name, {bool defaultValue = false}) {
    return GlobalVariableManager().getBoolVariableSync(
      name,
      defaultValue: defaultValue,
    );
  }
...
class GameState {
  ...
  GameState copyWith({
    ...
    return GameState(
      ...
      currentNode: forceNullCurrentNode
          ? null
      ...
      sceneAnimation: clearSceneAnimation
          ? null
          : (sceneAnimation ?? this.sceneAnimation),
      sceneAnimationRepeat: clearSceneAnimation
          ? null
          : (sceneAnimationRepeat ?? this.sceneAnimationRepeat),
      animeOverlay: clearAnimeOverlay
          ? null
          : (animeOverlay ?? this.animeOverlay), // 新增
...
class DialogueHistoryEntry {
  ...
  final Uint8List _packedStateSnapshot;
  ...
  DialogueHistoryEntry({
    ...
    this.sourceLine,
    required GameStateSnapshot stateSnapshot,
  }) : _packedStateSnapshot = HistorySnapshotCodec.pack(
         BinarySerializer.serializeGameStateSnapshot(stateSnapshot),
       );

  GameStateSnapshot get stateSnapshot =>
      BinarySerializer.deserializeGameStateSnapshot(
        HistorySnapshotCodec.unpack(_packedStateSnapshot),
      );

  Uint8List get serializedStateSnapshot =>
      HistorySnapshotCodec.unpack(_packedStateSnapshot);

  int get packedSnapshotBytes => _packedStateSnapshot.length;
}
