...
fn decode_header(bytes: &[u8], file_path: String) -> Result<RustSaveHeader, String> {
    let mut reader = Reader::new(bytes);
    if reader.exact(4)?.as_slice() != MAGIC {
        return Err("invalid SAKI save magic".to_string());
    }
...
pub fn scan_save_headers(
    ...
    for path in paths {
        ...
        match fs::read(&path)
            .map_err(|error| format!("read save file: {error}"))
            .and_then(|bytes| decode_header(&bytes, file_path.clone()))
        {
            Ok(slot) => slots.push(slot),
            Err(error) => invalid_files.push(format!("{file_path}: {error}")),
        }
    }
